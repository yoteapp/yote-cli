...
import {
  ...
  , add__PascalNamePlural__ToList
  , remove__PascalNamePlural__FromList
  , fetchSingleIfNeeded
 ...
 * @param {string} endpoint - The specific endpoint to hit when creating the __camelName__ (optional)
 * @param {string} method - The http method to use when creating the __camelName__ (optional, defaults to POST)
 * 
 ...
 * )
 */
export const useCreate__PascalName__ = ({ initialState = {}, onResponse = () => { }, endpoint, method }) => {
  ...
  const sendMutation = (mutated__PascalName__) => dispatch(sendCreate__PascalName__({ endpoint, method, ...mutated__PascalName__ }));

  // the hook will return everything the caller needs to create a new __camelName__
  return useMutateResource({ resourceQuery: default__PascalName__Query, sendMutation, initialState, onResponse, isCreate: true });
}
...
export const useGet__PascalName__ = (...args) => {
  ...
  const fetch__PascalName__ = fetchSingle__PascalName__AtEndpoint;
...
export const useGet__PascalName__List = (...args) => {
  ...
  const addToList = (queryString, __camelName__Ids) => dispatch(add__PascalNamePlural__ToList({ queryString, ids: __camelName__Ids }))
  const removeFromList = (queryString, __camelName__Ids) => dispatch(remove__PascalNamePlural__FromList({ queryString, ids: __camelName__Ids }))

  // return the (now __camelName__ specific) hook
  return useGetResourceList({ listArgs, fromStore: __camelName__Store, sendFetchList, sendInvalidateList, endpoint, addToList, removeFromList });
}
 ...
 * const { sendUpdate__PascalName__ } = useUpdate__PascalName__({ endpoint: myEndpoint, method: 'POST' });
 * // dispatch the update action
 * sendUpdate__PascalName__(updated__PascalName__);
 */
export const useUpdate__PascalName__ = ({endpoint, method}) => {
  const dispatch = useDispatch();
  return {
    // return the update action
    sendUpdate__PascalName__: (updated__PascalName__) => dispatch(sendUpdate__PascalName__({ endpoint, method, ...updated__PascalName__}))
  }
}
 ...
 * - `handleChange`: standard form change handler to be used in the form
 * - `handleSubmit`: standard form submit handler to be used in the form
 * - `isChanged`: a boolean that is true if the __camelName__ has pending changes that have not yet been saved via handleSubmit
 * - `setFormState`: a way to handle form state changes in the component instead of `handleChange`, rarely needed but sometimes necessary
 * @example // to use in a component
...
export const useGetUpdatable__PascalName__ = (id, { onResponse = () => { } } = {}) => {
  ...
  // return the (now __camelName__ specific) hook
  return useMutateResource({ resourceQuery: __camelName__Query, sendMutation, onResponse, id });
}
