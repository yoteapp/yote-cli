...
// First define all standard CRUD API calls for __camelName__
...
// CREATE
/**
 * @param {object} args - the new __camelName__ object plus the optional endpoint and method
...
export const sendCreate__PascalName__ = createAsyncThunk(
  ...
  , async ({ endpoint, method = 'POST', ...new__PascalName__ }) => {
    endpoint = endpoint ? `/api/__kebabNamePlural__/${endpoint}` : `/api/__kebabNamePlural__`;
    const response = await apiUtils.callAPI(endpoint, method, new__PascalName__);
...
// UPDATE
/**
 * @param {object} args - the updated __camelName__ object plus the optional endpoint and method
 * @param {string} args.endpoint - the endpoint to send the update to (optional)
...
export const sendUpdate__PascalName__ = createAsyncThunk(
  ...
  , async ({ endpoint, method = 'PUT', ...updates }) => {
    const { _id } = updates;
    endpoint = endpoint ? `/api/__kebabNamePlural__/${endpoint}` : `/api/__kebabNamePlural__/${_id}`;
...
export const __camelName__Slice = createSlice({
  ...
  , reducers: {
    ...
    , add__PascalNamePlural__ToList: handleAddManyToList
    , remove__PascalNamePlural__FromList: handleRemoveManyFromList
  }
...
export const { invalidateQuery, invalidateQueries, add__PascalName__ToList, add__PascalNamePlural__ToList, remove__PascalNamePlural__FromList } = __camelName__Slice.actions;


// We can also write thunks by hand, which may contain both sync and async logic.
// Here's an example of conditionally dispatching actions based on current state.
// accepts an optional listFetch action so we can use it for other list fetches, defaults to fetch__PascalName__List
export const fetchListIfNeeded = (queryKey, listFetch = fetch__PascalName__List) => (dispatch, getState) => {
  ...
  return handleFetchListIfNeeded(dispatch, __camelName__Store, listFetch, queryKey, '__camelNamePlural__');
};

// accepts an optional singleFetch action so we can use it for other single fetches, defaults to fetchSingle__PascalName__
export const fetchSingleIfNeeded = (id, singleFetch = fetchSingle__PascalName__) => (dispatch, getState) => {
  const __camelName__Store = getState().__camelName__;
  return handleFetchSingleIfNeeded(dispatch, __camelName__Store, singleFetch, id);
}
